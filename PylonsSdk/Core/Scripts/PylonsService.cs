...
namespace PylonsSdk
{
    ...
    {
        public class RunOnServiceLive : Attribute
        {
        ...
        void Initialize()
        {
            ...
            instance = this;
            RetrieveOnServiceLiveFuncs();
            if (Application.isPlaying) DontDestroyOnLoad(this);
            onServiceLive?.Invoke(this, EventArgs.Empty);
        }

        /// <summary>
        /// Gets all public static methods in any loaded assemblies w/ the RunOnServiceLive attribute and adds them to
        /// the queue.
        /// This is extremely slow, since it iterates over every type in the project, but only has to be run once.
        /// </summary>
        void RetrieveOnServiceLiveFuncs()
        {
