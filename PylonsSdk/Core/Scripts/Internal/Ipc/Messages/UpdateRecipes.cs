...
{
    /// <summary>
    /// IPC: Batch operation. Updates one or more recipes (sequentially, not simultaneously) using the linked wallet.
    /// Linked wallet must have a keypair. Note that UpdateRecipes should never be called in production code; end-user
    /// wallets are not able to send the cookbook/recipe creation/maintenance messages.
    /// 
    /// In general, UpdateRecipes is designed to be called by the automated, human-friendly recipe builder provided with the
    ...
    /// (Note that we don't have a non-batch UpdateRecipe message, to cut down on redundant code. If you want to create
    /// a single recipe, just use arrays of length 1. Contractually speaking, updating *zero* recipes should work, though
    /// obviously that should never actually be done.)
    /// 
    /// Gets a TxResponse containing all of the created UpdateRecipe transactions.
    /// </summary>
    public sealed class UpdateRecipes : IpcMessage
    {
        /// <summary>
        /// An array containing the unique-within-cookbook IDs of the recipes to be updated. If these don't exist, the operation will fail.
        /// </summary>
        [JsonProperty("ids")]
        ...
        /// <summary>
        /// An array containing the description strings for the recipes to be updated.
        /// </summary>
        ...
        public UpdateRecipes(string[] ids, string[] names, string[] cookbooks, string[] decriptions, long[] blockIntervals, string[] coinInputs, 
            string[] itemInputs, string[] outputTables, string[] outputs) : base(ResponseType.TX_RESPONSE)
        {
            Ids = ids;
            Names = names;
            Cookbooks = cookbooks;
